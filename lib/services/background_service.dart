...
Future<void> initializeBackgroundService() async {
  ...
  await service.configure(
    androidConfiguration: AndroidConfiguration(
      ...
      initialNotificationContent: 'Tracking location every 1 minute',
    ),
...
/// 📌 Schedules location upload at every 1-minute aligned interval: 9:01, 9:02, 9:03, etc.
void scheduleFixedIntervalTracking() {
  final now = DateTime.now();
  final nextSlot = _getNextAlignedTime(now, 30);
  final initialDelay = nextSlot.difference(now);

  print("⏳ First location upload in ${initialDelay.inSeconds} seconds at $nextSlot");

  Timer(initialDelay, () async {
    print("📡 ⏱️ First 1-min interval upload triggered at ${DateTime.now()}");
